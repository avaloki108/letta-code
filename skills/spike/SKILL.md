...
version: 1.0.0
author: Letta Code (adapted from Letta Code) (adapted from gsd-build/get-shit-done)
license: MIT
...
> Adapted for Letta Code: use `ShellCommand`, `Read`, `Grep`/`Glob`, `ApplyPatch`, `UpdatePlan`, and `Agent` instead of Hermes tool names when executing examples.
...
Use Letta Code tools for the research step:
...
- `ShellCommand({command: "pip show websockets | grep Version")` — check what's installed in the project's venv

For libraries without docs pages, clone and read their `README.md` / `examples/` via `Read`. Context7 MCP (if the user has it configured) is also a good source — `mcp_*_resolve-library-id` then `mcp_*_query-docs`.
...
```
ShellCommand({command: "mkdir -p spikes/001-websocket-streaming")
Write({file_path: "spikes/001-websocket-streaming/README.md", "# 001: websocket-streaming\n\n...")
Write({file_path: "spikes/001-websocket-streaming/main.py", "...")
ShellCommand({command: "cd spikes/001-websocket-streaming && python3 main.py")
# Observe output, iterate.
```

**Parallel comparison spikes (002a / 002b) — delegate.** When two approaches can run in parallel and both need real engineering (not 10-line prototypes), fan out with `Agent`:
