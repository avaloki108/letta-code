...
describe("composeSubagentChildEnv", () => {
  ...
  test("does NOT compose transitive scope from parent's LETTA_MEMORY_SCOPE", () => {
    // Subagents can't spawn subagents (no builtin has Task in its tools
    // list). The parent process therefore never has LETTA_MEMORY_SCOPE
    // set by our own code. If an external actor set it, we must ignore
    // it — the child's scope is exactly {parent}, no broader ancestor
    // chain.
    const env = composeSubagentChildEnv({
      parentProcessEnv: {
        HOME: "/home/user",
    ...
    expect(env.LETTA_MEMORY_SCOPE).toBe(PARENT_ID);
    expect(env.LETTA_MEMORY_SCOPE).not.toContain("agent-grandparent");
    expect(env.LETTA_MEMORY_SCOPE).not.toContain("agent-random");
  });
