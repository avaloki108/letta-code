 ...
 *
 *   - LETTA_MEMORY_SCOPE always includes the parent's agent ID when one is
 *     known. Every subagent is a subordinate of its parent; the cross-agent
 *     memory guard shouldn't treat that relationship as a foreign-agent
 *     intrusion. This applies to explore/general-purpose/recall etc. — not
 *     just memory-writing subagents.
 *
...
export function composeSubagentChildEnv(
  ...
  // Authorize the subagent to cross into the parent's memory. Independent
  // of permissionMode — Read from parent memory is a legitimate operation
  // for any subagent type, and the cross-agent guard would otherwise deny
  // it as a foreign-agent access.
  if (parentAgentId) {
    childEnv.LETTA_MEMORY_SCOPE = parentAgentId;
  } else {
